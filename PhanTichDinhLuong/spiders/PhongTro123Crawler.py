...
class PhongTro123Spider(scrapy.Spider):
    ...
    def parse_room_detail(self, response):
        listFeatures = [" Đầy đủ nội thất ", " Giờ giấc tự do ", " Có máy giặt ", " Có tủ lạnh ", " Có gác ", " Có máy lạnh ", " Không chung chủ "]
        checkFeatures = response.xpath('//i[@class="icon check-circle-fill green me-2"]/following-sibling::text()').getall()
        
        item = {
        'features_freetime': 0,
        'features_ownerless': 0,
        'features_washer': 0,
        'features_refrigerator': 0,
        'features_airConditioning': 0,
        'features_Attic': 0,
        'features_interiorFull': 0
        }

        if not checkFeatures:
            return
        else:
            for feature in checkFeatures:
                if feature not in listFeatures:
                    return
                else:
                    if feature in listFeatures:
                        if feature == "Giờ giấc tự do":
                            item['features_freetime'] = 1
                        elif feature == "Không chung chủ":
                            item['features_ownerless'] = 1
                        elif feature == "Có máy giặt":
                            item['features_washer'] = 1
                        elif feature == "Có tủ lạnh":
                            item['features_refrigerator'] = 1
                        elif feature == "Có gác":
                            item['features_Attic'] = 1
                        elif feature == "Có máy lạnh":
                            item['features_airConditioning'] = 1
                        elif feature == "Đầy đủ nội thất":
                            item['features_interiorFull'] = 1
